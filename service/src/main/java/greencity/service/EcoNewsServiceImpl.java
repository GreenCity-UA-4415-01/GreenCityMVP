...
import greencity.util.NotificationEventBuilder;
...
public class EcoNewsServiceImpl implements EcoNewsService {
    ...
    public void like(UserVO userVO, Long id) {
        ...
        boolean alreadyLiked = ecoNewsVO.getUsersLikedNews().stream().anyMatch(u -> u.getId().equals(userVO.getId()));
        if (alreadyLiked) {
            ecoNewsVO.getUsersLikedNews().removeIf(u -> u.getId().equals(userVO.getId()));
        } else {
            ecoNewsVO.getUsersLikedNews().add(userVO);
            // ----- NEW NOTIFICATION EMISSION LOGIC BEGIN -----
            // Notify only if liker is not the author
            if (ecoNewsVO.getAuthor() != null && !ecoNewsVO.getAuthor().getId().equals(userVO.getId())) {
                NotificationEventMessage event = NotificationEventBuilder.createForSingleRecipient(
                        ecoNewsVO.getAuthor().getId(),
                        List.of(userVO.getId()),
                        NotificationActionType.LIKE_CREATED,
                        NotificationObjectType.NEWS,
                        ecoNewsVO.getId().toString(),
                        ecoNewsVO.getTitle()
                );
                notificationProducerService.sendNotificationEvent(event, NotificationRoutingKeys.NEWS_LIKE_CREATED);
            }
            // ----- NEW NOTIFICATION EMISSION LOGIC END -----
        }
