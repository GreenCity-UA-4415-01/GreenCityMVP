...
class EventControllerTest {
    ...
    void getMyEventsWithStatusFilter_ShouldReturnFilteredEvents() throws Exception {
        ...
        when(eventService.getMyEvents(eq(5L), eq(null), eq(EventStatus.UPCOMING), eq(null), eq(null),
            any(Pageable.class)))
            .thenReturn(eventPage);
...
//                        .organizerId(11L)
    ...
    @Test
    void searchEvents_ShouldReturn400_TitleTooShort() throws Exception {
        mockMvc.perform(get("/events/search")
            .param("title", "ec")
            .accept(MediaType.APPLICATION_JSON))
            .andExpect(status().isBadRequest())
    ...
    @Test
    void searchEvents_ShouldReturn200WithEmptyList_NoResultsFound() throws Exception {
