...
class HabitAssignControllerTest {
    ...
    void setup() {
        ...
        habit = HabitDto.builder()
                ...
                .customShoppingListItems(List.of(
                        CustomShoppingListItemResponseDto.builder().id(101L).text("Придбати контейнери для сортування").status(ShoppingListItemStatus.INPROGRESS).build()
                ))
                .isCustomHabit(false)
    ...
    void testAssignHabitWithCustomProperties() throws Exception {
        ...
        mockMvc.perform(post("/habit/assign/" + habitId + "/custom")
                ...
                .andExpect(jsonPath("$[0].progressNotificationHasDisplayed").value(habitAssignManagement.getProgressNotificationHasDisplayed()));
    }
    ...
    void testGetUserShoppingAndCustomShoppingLists() throws Exception {
        ...
        mockMvc.perform(get("/habit/assign/allUserAndCustomShoppingListsInprogress")
                ...
                .andExpect(jsonPath("$[0].userShoppingListItemDto[0].status").value(userShoppingResponse.getStatus().toString()))
                ...
                .andExpect(jsonPath("$[0].customShoppingListItemDto[0].status").value(customShoppingResponse.getStatus().toString()));
    }
    ...
    void testGetMoreHabitAssignByHabitId() throws Exception {
        ...
        mockMvc.perform(get("/habit/assign/" + habitAssignId + "/more")
                ...
                .andExpect(jsonPath("$.customShoppingListItems[0].id").value(habit.getCustomShoppingListItems().get(0).getId()))
                .andExpect(jsonPath("$.customShoppingListItems[0].text").value(habit.getCustomShoppingListItems().get(0).getText()))
                .andExpect(jsonPath("$.customShoppingListItems[0].status").value(habit.getCustomShoppingListItems().get(0).getStatus().toString()))
